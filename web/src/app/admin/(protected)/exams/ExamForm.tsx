...
export default function ExamForm({ mode, exam }: { mode: "create" | "edit"; exam?: Exam }) {
  const router = useRouter();

  const [name, setName] = useState(exam?.name ?? "");
  const [order, setOrder] = useState(exam?.order ?? 0);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const res = await fetch(mode === "create" ? "/api/exams" : `/api/exams/${exam!.id}`, {
  ...
  return (
    <div className="max-w-2xl space-y-6 pt-2">
      <h1 className="text-2xl font-semibold text-argon-dark">
        {mode === "create" ? "Novo exame" : `Editar: ${exam?.name}`}
      </h1>
